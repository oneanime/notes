create
    definer = root@`%` procedure SP_BILL_APPLY_GRADE_DEBT_PRI(IN APPLYID varchar(100), IN IDCARD varchar(20),
...
BEGIN
    ...
    set VV_TASK = '将对私账户中 民间借贷类隐形负债流水明细 入中间表';
    ...
    where (OTHER_NAME like '%小贷%' or OTHER_NAME like '%金融信息%' or OTHER_NAME like '%消费金融%' or OTHER_NAME like '%信息咨询%' or
      ...
      and (ORG_CODE in ('alipay', 'alipaysh', 'wechat', 'wechatsh') or
           (ORG_CODE not in ('alipay', 'alipaysh', 'wechat', 'wechatsh') and ACCOUNT_TYPE = 2))
      and EXPEND_AMOUNT > 0
    ...
    where abs(a.EXPEND_AMOUNT - b.EXPEND_AMOUNT) <= 100
      ...
      and not exists(
            select 1
            from report_loan_info c
            where abs(a.EXPEND_AMOUNT - c.MONTHLY_PAYMENT) <= 100
              and c.APPLY_ID = APPLYID
