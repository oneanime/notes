create
    definer = root@`%` procedure SP_DATA_CUST_ASSET_HOUSE(IN APPLYID varchar(50), IN SEQNO varchar(50),
...
BEGIN
    ...
    IF SEQNO <> '' AND IDCARD <> '' AND AREACODE <> '' AND APPLYID = '' AND PTYPE = '' THEN
        ...
        -- 申请人房产数据去重处理
        CALL SP_DATA_CUST_ASSET_HOUSE_INSERT(APPLYID, SEQNO, IDCARD, PTYPE, AREACODE, DATATYPE);

        -- 申请人配偶房产数据去重处理
        IF V_CP_ID_CARD <> '' THEN
            CALL SP_DATA_CUST_ASSET_HOUSE_INSERT(APPLYID, SEQNO, V_CP_ID_CARD, PTYPE, AREACODE, DATATYPE);
        END IF;
    END IF;
