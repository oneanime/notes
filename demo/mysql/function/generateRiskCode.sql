create
    definer = root@`%` function generateRiskCode(areacode varchar(20)) returns varchar(32)
BEGIN
    ...
    SET initCode='000001';
    SET con=0;
    SELECT CONCAT(DATE_FORMAT(CURDATE(),'%y%m%d'),areacode,'77') INTO cur_date FROM dual;
    SELECT MAX(SEQ_NO) INTO seqNo FROM POSTLOAN_RISK_HEADSUP WHERE AREA_CODE=areacode   and SEQ_NO LIKE  CONCAT(cur_date,'%');
