create
    definer = root@`%` function generatePostLoanCode(areacode varchar(20)) returns varchar(32)
BEGIN
    ...
    SELECT MAX(SEQ_NO) INTO seqNo FROM POSTLOAN_CUST_STAT WHERE AREA_CODE=areacode and SEQ_NO LIKE  CONCAT(cur_date,'%');
