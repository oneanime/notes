create
    definer = root@`%` function generateApplyCode(areacode varchar(20)) returns varchar(32)
BEGIN
    ...
    SET initCode='000001';
    SET con=0;
    SELECT CONCAT(DATE_FORMAT(CURDATE(),'%y%m%d'),areacode) INTO cur_date FROM dual;
    SELECT MAX(APPLY_ID) INTO applyId FROM APPLY_CREDIT WHERE AREA_CODE=areacode and APPLY_ID LIKE  CONCAT(cur_date,'%');
