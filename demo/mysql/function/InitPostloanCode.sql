create
    definer = root@`%` function InitPostloanCode(idcard varchar(20), areacode varchar(20)) returns varchar(32)
BEGIN
    ...
    SELECT CONCAT(DATE_FORMAT(CURDATE(),'%y%m%d'), areacode ,'88') INTO cur_date FROM dual;
    SELECT RN INTO VI_RN FROM postloan_cust_tmp WHERE ID_CARD=idcard ;
    SELECT LENGTH(RN) INTO VI_LEN FROM postloan_cust_tmp WHERE ID_CARD=idcard ;
