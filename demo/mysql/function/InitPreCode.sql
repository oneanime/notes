create
    definer = root@`%` function InitPreCode(idcard varchar(20), areacode varchar(20)) returns varchar(32)
BEGIN
    ...
    SELECT CONCAT(DATE_FORMAT(CURDATE(),'%y%m%d'), areacode ,'99') INTO cur_date FROM dual;
    SELECT RN INTO VI_RN FROM pre_credit_cust_tmp WHERE ID_CARD=idcard ;
    SELECT LENGTH(RN) INTO VI_LEN FROM pre_credit_cust_tmp WHERE ID_CARD=idcard ;
